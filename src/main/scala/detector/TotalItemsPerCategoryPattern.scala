...
object TotalItemsPerCategory {

	/**
	  * Tests for a pattern in the total purchase amount (including quantity) by product category.
	  *
	...
	def Go(data: DataFrame): Option[String] = {
		...
		if (ndev > 1.0 + PatternDetector.marginOfError) {  // Pattern detected
			val filename = PatternDetector.saveDataFrameAsCSV(newDf, "TotalOrdersPerCategory.csv")  // Write the data to a file
			if (ndev < 2)
