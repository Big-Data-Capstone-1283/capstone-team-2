...
object OrdersByHourOfDayPattern {
	...
	/**
	  * Tests for a pattern in the purchase frequency by time of day on the "datetime" column in the dataframe.  (Assumes pre-normalized time zone data.)
	  *
	...
	def Go(data: DataFrame): Option[String] = {
		var newDf = data  // Generate the data
			...
			.groupBy("hour_of_day")
			.agg(count("hour_of_day").as("count"))
		...
		if (ndev > 1.0 + PatternDetector.marginOfError) {  // Pattern detected
			val filename = PatternDetector.saveDataFrameAsCSV(newDf, "HourOfDayRates.csv")  // Write the data to a file
			if (ndev < 2)
