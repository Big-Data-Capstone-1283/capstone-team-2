...
object WebsiteByDayOfWeekPattern {

	/**
	  * Tests for a pattern in the purchase frequency by website and day of week.  (Assumes pre-normalized time zone data.)
	  *
	...
	def Go(data: DataFrame): Option[String] = {
		var newDf = data  // Generate the "average_count" and "average_total" data
			...
			.groupBy("ecommerce_website_name", "day_of_week")
		...
		var newDfSucc = data  // Generate the "average_total_successful" data
			...
			.groupBy("temp_website", "temp_day_of_week")
		...
		newDf = newDf  // Merge the two dataframes
			.join(newDfSucc, newDf("day_of_week") === newDfSucc("temp_day_of_week") && newDf("ecommerce_website_name") === newDfSucc("temp_website"), "full")
			.drop("temp_day_of_week", "temp_website")
			.orderBy(col("ecommerce_website_name"), col("daynum_of_week"))
		...
		if (ndev > 1.0 + PatternDetector.marginOfError) {  // Pattern detected
			filename = PatternDetector.saveDataFrameAsCSV(newDf, "WebsiteByDayOfWeek.csv")  // Write the data to a file
			if (ndev < 2)
