...
object ProdCat_WebsitePattern {
	...
	def Go(data: DataFrame): Option[String] = {
		var newDf = data  // Generate the "count" and "total" data
			.select("product_category", "ecommerce_website_name", "qty")
			.groupBy("product_category", "ecommerce_website_name")
			.agg(count("product_category").as("count"), sum("qty").as("total"))
		var newDfSucc = data  // Generate the "total_successful" data
			.select(col("product_category").as("temp_product_category"), col("ecommerce_website_name").as("temp_website"), col("qty"))
			.where("payment_txn_success = 'Y'")
			.groupBy("temp_product_category", "temp_website")
			.agg(sum("qty").as("total_successful"))
		newDf = newDf  // Merge the two dataframes
			.join(newDfSucc, newDf("product_category") === newDfSucc("temp_product_category") && newDf("ecommerce_website_name") === newDfSucc("temp_website"), "full")
			.drop("temp_product_category", "temp_website")
			.orderBy("product_category", "ecommerce_website_name")
		...
		if (ndev > 1.0 + PatternDetector.marginOfError) {  // Pattern detected
			filename = PatternDetector.saveDataFrameAsCSV(newDf, "ProdCat_WebsiteRates.csv")  // Write the data to a file
			if (ndev < 2)
