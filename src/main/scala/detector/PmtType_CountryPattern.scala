...
object PmtType_CountryPattern {
	...
	def Go(data: DataFrame): Option[String] = {
		var newDf = data  // Generate the "count" and "total" data
			.select("payment_type", "country", "qty")
		...
		var newDfSucc = data  // Generate the "total_successful" data
			.select(col("payment_type").as("temp_payment_type"), col("country").as("temp_country"), col("qty"))
			.where("payment_txn_success = 'Y'")
			.groupBy("temp_payment_type", "temp_country")
			.agg(sum("qty").as("total_successful"))
		newDf = newDf
			.join(newDfSucc, newDf("payment_type") === newDfSucc("temp_payment_type") && newDf("country") === newDfSucc("temp_country"), "full")
			.drop("temp_payment_type", "temp_country")
		...
		if (ndev > 1.0 + PatternDetector.marginOfError) {  // Pattern detected
			filename = PatternDetector.saveDataFrameAsCSV(newDf, "PmtType_CountryRates.csv")  // Write the data to a file
			if (ndev < 2)
