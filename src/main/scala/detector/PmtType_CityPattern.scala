...
object PmtType_CityPattern {
	...
	def Go(data: DataFrame): Option[String] = {
		var newDf = data  // Generate the "count" and "total" data
			.select("payment_type", "city", "qty")
			.groupBy("payment_type", "city")
			.agg(count("payment_type").as("count"), sum("qty").as("total"))
		var newDfSucc = data  // Generate the "total_successful" data
			.select(col("payment_type").as("temp_payment_type"), col("city").as("temp_city"), col("qty"))
			.where("payment_txn_success = 'Y'")
			.groupBy("temp_payment_type", "temp_city")
			.agg(sum("qty").as("total_successful"))
		newDf = newDf
			.join(newDfSucc, newDf("payment_type") === newDfSucc("temp_payment_type") && newDf("city") === newDfSucc("temp_city"), "full")
			.drop("temp_payment_type", "temp_city")
			.orderBy("payment_type", "city")
